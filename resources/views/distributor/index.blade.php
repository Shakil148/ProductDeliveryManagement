@extends('layouts.master')

@section('content')
<div class="container mt-2">
    <div class="row justify-content-center">
        <div class="col-md-16">
            <div class="card">
            <h3 class="display-5 text-center mt-2 bg-secondary">Distributor List</h3>
    <div>
    <a style="margin: 19px;" href="{{ route('distributor.create')}}" class="btn btn-primary">New Distributor</a>
    </div>
 <div class="table-hover table-striped table-bordered ">
  <table class="table table-responsive fixed-table-body">
    <thead class="bg-dark">
        <tr>
          <td>#</td>
          <td>Name</td>
          <td>Contact</td>
          <td>Car No</td>
          <td colspan = 2 class="text-center">Actions</td>
        </tr>
    </thead>
    
    <tbody>
        @foreach($distributor as $distributorlist)
        <tr class="table-info">
            <td>{{$loop->iteration}}</td>
            <td>{{$distributorlist->name}}</td>
            <td>{{$distributorlist->contact}}</td>
            <td>{{$distributorlist->carNo}}</td>

            <td>
                <a href="{{ route('distributor.edit',$distributorlist->id)}}" class="btn btn-primary"><i class="fa fa-edit"></i></a>
            </td>
            <td>
                <form onclick="return confirm('Are you sure?')" 
                action="{{ route('product.destroy', $distributorlist->id)}}" method="post">
                  @csrf
                  @method('DELETE')
                  <button class="btn btn-danger" type="submit"><i class="fa fa-trash"></i></button>
                </form class ="mb-2">
            </td>
        </tr>
        @endforeach
    </tbody>
  </table>
    {{$distributor->links()}}
  </div>


            </div>
        </div>
    </div>
</div>
@endsection
